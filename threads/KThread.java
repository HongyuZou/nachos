...
public class KThread {
	...
	public static void finish() {
		...
		// if parent exists, put it in ready queue
		KThread parent = currentThread().parent;
		if(parent != null) {
			parent.ready();
		}
	...
	public void join() {
		...
		// if not finished, block parent thread
		if(this.status != statusFinished) {
			KThread parentThread = currentThread();
	...
	public static void joinTest3 () {
		KThread parent = currentThread();
		KThread child1 = new KThread( new Runnable () {
			public void run() {
				System.out.println("I (heart) Nachos!");
				System.out.println("is child joined " + currentThread().isJoined);
				System.out.println("parent status: " + parent.status);
			}
			});
		
		KThread child2 = new KThread( new Runnable () {
			public void run() {
				System.out.println("join child 1 again");
				child1.join();
			}
			});
		...
		child1.join();
		System.out.println("parent 1 finished");
	}
	...
	public static void joinTest5 () {
		...
		KThread child1 = new KThread( new Runnable () {
			public void run() {
				...
				ThreadedKernel.alarm.waitUntil(10000);
				System.out.println("is child joined 1" + currentThread().isJoined);
				System.out.println("parent status: " + parent.status);
			}
		});
		...
		KThread child2 = new KThread( new Runnable () {
			public void run() {
				...
				System.out.println("is child joined 2" + currentThread().isJoined);
				System.out.println("parent status: " + parent.status);
			}
		});
